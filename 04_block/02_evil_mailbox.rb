...
class EvilMailbox
  def initialize(mailbox, secret = nil)
    ...
    define_singleton_method :send_mail do |recipient, text, &block|
      result = @mailbox.send_mail(recipient, text + secret.to_s)
...
#     text += @secret unless @secret.nil?

#     result = @mailbox.send_mail(recipient, text)
